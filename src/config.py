...
def get_prune_config():
    parser = argparse.ArgumentParser("Visual Transformer Pruning")
    # basic config
    ...
    parser.add_argument("--model-arch", type=str, default="b16", help='model setting to use', choices=['ti16', 's16', 'b16', 'b32', 'l16', 'l32', 'h14', 'deit_base_patch16_224'])
    parser.add_argument("--checkpoint-path", type=str, default='../experiments/save/vb16-wd1e-4_CIFAR100_bs16_lr0.001_wd0.0001_210408_142954/checkpoints/best.pth', help="model checkpoint to load weights")
    ...
    parser.add_argument("--prune_mode", type=str, default='seq', help='choose which mode to prune',
                        choices=['seq', 'head', 'attn', 'random', 'head_entropy', 'hrand'])
    parser.add_argument("--prune_rate", type=float, default=0.25)
    parser.add_argument("--iter_nums", type=int, default=24)
    parser.add_argument("--final_finetune", type=int, default=6)
...
def get_eval_config():
    ...
    # basic config
    parser.add_argument("--n-gpu", type=int, default=1, help="number of gpus to use")
    parser.add_argument("--model-arch", type=str, default="b32", help='model setting to use',
                        choices=['ti16', 's16', 'b16', 'b32', 'l16', 'l32', 'h14'])
    parser.add_argument("--checkpoint-path", type=str, default='../experiments/save/vb16-wd1e-4_CIFAR100_bs16_lr0.001_wd0.0001_210408_142954/checkpoints/best.pth', help="model checkpoint to load weights")
    parser.add_argument("--image-size", type=int, default=384, help="input image size", choices=[224, 384])
...
def get_train_config():
    ...
    parser.add_argument("--model-arch", type=str, default="b16",
                        help='model setting to use', choices=['ti16', 's16', 'b16', 'b32', 'l16', 'l32', 'h14'])
...
def get_deit_base_patch16_224_config(config):
...
def get_ti16_config(config):
    """ ViT-Ti/16 configuration """
    config.patch_size = 16
    config.emb_dim = 192
    config.mlp_dim = 768
    config.num_heads = 3
    config.num_layers = 12
    config.block_settings = [192] * 12
    config.attn_dropout_rate = 0.0

...
from utils import setup_device, accuracy, MetricTracker, TensorboardWriter, log_attns


def train_epoch(epoch, model, data_loader, criterion, optimizer, metrics, lr_scheduler=None, device=torch.device('cpu')):
    metrics.reset()
    model.to(device)
    # training loop
...
def save_model(save_dir, epoch, model, optimizer, lr_scheduler, device_ids, block_settings=[768]*12, best=False):
    state = {
        'epoch': epoch,
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        # train the model
        model.train()
        result = train_epoch(epoch, model, train_dataloader, criterion, optimizer, train_metrics, lr_scheduler,  device)
        # log_attns(model, epoch)
        log.update(result)
        ...
        save_model(config.checkpoint_dir, epoch, model, optimizer,
                   lr_scheduler, device_ids, config.block_settings, best)

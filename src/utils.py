...
def log_attns(model, epoch):
    ...
    for attn_map, cnt in zip(attn_maps, iter_cnts):
        ...
        mean_map = (attn_map.sum(dim=0) / (b * cnt)).data
        mean_maps.append(np.asarray(mean_map))
    with open('./experiments/cifar10/epoch{}.pkl'.format(epoch), 'wb') as f:

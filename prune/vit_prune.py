...
def get_new_qkv(module, head, head_dim, idx):
    ...
    new_qkv = nn.Linear(in_dim, new_out_dim * 3)
    new_qkv.weight.data = new_weight.reshape(-1, in_dim)

    # print(new_qkv.weight.data.shape)
...
def prune_tokens(model, masks):
    ...
    for name, module in model.named_modules():
        if isinstance(module, Attention):
            ...
            if module.seq_ranks is not None:
                mask = masks[layer]
...
def prune_attention(model, masks):
    ...
    layer = 0
    for name, module in model.named_modules():
        if isinstance(module, Attention) and module.attn is not None:
...
def prune(model, nums, mode='head_entropy'):
    ...
    # print(layer_ranks)
    layer_ranks = np.asarray([np.asarray(layer_rank.cpu()) * (1 - 0.015 * layer)

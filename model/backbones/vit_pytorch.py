...
class Attention(nn.Module):
    def __init__(self, dim, num_patches, num_heads, channel=None, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.in_dim = dim
        self.head_dim = DEFAULT_HEADDIM
        self.hidden_dim = num_heads * self.head_dim
        self.num_heads = num_heads
        # for simple parameter, set head_dim = 64

        # NOTE scale factor was wrong in my original version, can set manually to be compat with prev weights
        self.scale = qk_scale or self.head_dim ** -0.5

        self.qkv = nn.Linear(self.in_dim, self.hidden_dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(self.hidden_dim, self.in_dim)
        ...
        token_mask = torch.ones(num_patches + 1)
    ...
    def forward(self, x, compute_attn=None, compute_entro=None):
        B, N, C = x.shape
        # print('qkv:', qkv.weight.data.shape,)
    ...
    def flops(self):
        ...
        # q.k.v dot x
        flops += 3 * self.channel * self.hidden_dim * self.in_dim
        # attn = q matmul k.transpose
        flops += self.channel * self.hidden_dim * self.channel
        # softmax
        flops += self.num_heads * self.channel * self.channel
        # out = attn matmul v
        flops += self.channel * self.hidden_dim * self.channel
        # self.out dot out
        flops += self.channel * self.hidden_dim * self.in_dim
        return flops


class Block(nn.Module):

    def __init__(self, dim, num_heads, num_patches, mlp_ratio=4., channel=None, qkv_bias=False, qk_scale=None, drop=0.,
                 attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(
            dim, num_heads=num_heads, num_patches=num_patches, channel=channel,
    ...
    def forward(self, x, compute_attn=None, compute_entro=None):
        if not self.attn.disable_index:
            # print(self.attn.token_index.sum())
            token_keep = x[:, self.attn.token_index, :]
            token_res = x[:, ~self.attn.token_index, :]
            token_keep = token_keep + self.drop_path(
                self.attn(self.norm1(token_keep), compute_attn, compute_entro))
            token_keep = token_keep + self.drop_path(self.mlp(self.norm2(token_keep)))
            x = torch.cat([token_keep, token_res], dim=1)
        else:
            x = x + self.drop_path(self.attn(self.norm1(x), compute_attn=compute_attn, compute_entro=compute_entro))
    ...
    def flops(self):
        ...
        # print(self.attn.channel)
        flops += self.mlp.flops(self.attn.channel)
...
class TransReID(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, heads=None, depth=12,
                 num_heads=12, mlp_ratio=4., channels=None, qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., camera=0, view=0,
        ...
        if channels is None:
            channels = [self.num_patches + 1] * depth
        elif len(channels) == depth - 1:
            channels.append(self.num_patches + 1)

        if heads is None:
            heads = [num_heads] * depth
        elif len(heads) == depth - 1:
        ...
        self.blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=heads[i], num_patches=self.num_patches, channel=channels[i], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
    ...
    def forward_features(self, x, camera_id, view_id, compute_attn=None, compute_entro=None):
        B = x.shape[0]
        ...
        if self.cam_num > 0 and self.view_num > 0:
            ...
            # print(x.data.shape, self.pos_embed.data.shape, self.sie_embed[camera_id].data.shape)
        ...
        if self.local_feature:
            for blk in self.blocks[:-1]:
                x = blk(x, compute_attn=compute_attn, compute_entro=compute_entro)
            return x
    ...
    def forward(self, x, cam_label=None, view_label=None, compute_attn=None, compute_entro=None):
        x = self.forward_features(x, cam_label, view_label, compute_attn=compute_attn, compute_entro=compute_entro)
    ...
    def cal_seq(self):
        seq = torch.range(0, self.num_patches)
        for blk in self.blocks:
            if not blk.attn.disable_index:
                index = blk.attn.token_index
                seq = torch.cat([seq[index], seq[~index]], dim=0)

        idx, shuffle = torch.sort(seq)
...
def vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, channels=None, heads=None, **kwargs):
    model = TransReID(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  sie_xishu=sie_xishu, local_feature=local_feature, channels=channels, heads=heads, **kwargs)
